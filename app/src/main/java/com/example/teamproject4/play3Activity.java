...
public class play3Activity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle saveInstanceState) {
        ...
        if (a == 0) {
            t2.setText("상대가 먼저 공격합니다.\n구슬을 베팅하세요.");
        } else if (a == 1) {
            i.setImageDrawable(getResources().getDrawable(R.drawable.img));
            t2.setText("당신이 먼저 공격합니다.\n구슬을 베팅하세요.");
        }

        odd.setClickable(false);
        even.setClickable(false);//우선 홀 짝 버튼은 눌리면 안됨

        b.setOnClickListener(new View.OnClickListener() { //베팅하는 버튼.
            @Override
            public void onClick(View v) {
                EditText e = (EditText) findViewById(R.id.bet);
                n = Integer.parseInt(e.getText().toString()); //내가 베팅한 구슬 개수를 받음
                if (n > mine) { // 베팅한 구슬개수가 내가 가진거보다 큼
                    Toast.makeText(getApplicationContext(), "자신의 구슬 개수보다 많은 수의 구슬을 베팅할 수 없습니다.", Toast.LENGTH_SHORT).show();
                    odd.setClickable(false);
                ...
                if (com > 5) {//컴퓨터가 베팅할 구슬개수 랜덤으로 배정
                    combet = r.nextInt(5) + 1; // 1 2 3 4 5
                } else //컴퓨터가 가진 구슬 갯수가 5개 이하일때는 얘가 가진만큼에서만 랜덤함수 돌아감
                    combet = r.nextInt(com) + 1;

                //컴퓨터 공격일때
                if (a == 0 && n == minebet) {
                    com_b = r.nextInt(2);//0이면 짝, 1이면 홀을 골랐다고 치는거임
                    ...
                    odd.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            if (mine >= 20 || com >= 20) {
                                ...
                                i.setImageDrawable(getResources().getDrawable(R.drawable.com));
                                //t3.setText("컴퓨터의 공격");
                                Toast.makeText(getApplicationContext(), "상대의 공격 차례입니다. 구슬을 베팅하세요!", Toast.LENGTH_LONG).show();
                            }
                            odd.setClickable(false);
                            even.setClickable(false);
                        }
